+++
title = "もじゃもじゃ又はちくちくを作る"
date = "2009-09-15T00:00:00+09:00"
tags = ["indesign"]
+++
横溝正史氏の「悪魔の手毬唄」をイメージしてスクリプトを書いてみました。

![/images/2010/09/632-temari_mojamoja.jpg](/images/2010/09/632-temari_mojamoja.jpg)

円の中で型抜きしたみたいに、もじゃもじゃした感じのものになります。

パラメータのオプションでちくちくになります。
...
var mojaKoValue=15;//もじゃもじゃしたものの数
...
for(var i=0; i < mojaKoValue; i++){
  ...
  for(var ke=0; ke < lineValue; ke++){
    ...
    if(mojaORchiku == true){//もじゃもじゃ
      pCo=[
      //円の中心で方向線をランダムにひっぱる
        [[lineL*(Math.pow(-1,Math.round(Math.random())))*Math.random(),lineL*(Math.pow(-1,Math.round(Math.random())))*Math.random()],
        [0,0],
        [lineL*(Math.pow(-1,Math.round(Math.random())))*Math.random(),lineL*(Math.pow(-1,Math.round(Math.random())))*Math.random()]],
