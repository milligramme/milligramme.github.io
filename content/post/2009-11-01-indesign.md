+++
title = "選択オブジェクトをグリグリ、ゲジゲジ"
date = "2009-11-01T00:00:00+09:00"
tags = ["extendscript", "indesign"]
+++
InDesignで選択したオブジェクトの形状に沿って、ゲジゲジまたはグリグリとトレースしてみる試み。

たとえば鉛筆ツールでつつつーっと描いてゲジゲジ実行。

アンカーポイントを直線的に追従しながらランダムにゲジゲジ。

実行後に角オプションを適用するとクシュクシュになったりします。

楕円はアンカーポイントが4ヶ所のみなので実行すると菱形状になってしまうので、代わりに多角形で「頂点の数多め（２０とか）に、星形の比率を0%」に実行すると丸っぽくできます。グリグリでは角度を小さくするほど丸っぽく、90°付近で四角形ぽく、最大の120°では三角形っぽくグリグリンと。360°を整数で割れない角度のほうが面白いかも。

元のオブジェクトはそのまま残すので、納得いくまでトライアンドエラーで。

![/images/2010/09/663-grin_and_gezy.png](/images/2010/09/663-grin_and_gezy.png)
...
var grinRadioB = radiobuttonControls.add({staticLabel:"grin grin", checkedState:true});
var gezyRadioB = radiobuttonControls.add({staticLabel:"gezy gezy"});
}
}
}
}
